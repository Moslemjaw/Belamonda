...
// Finance snapshot (SRS §5.4)
reportingRouter.get("/finance/snapshot", authRequired, requireRole(["finance", "admin"]), (_req, res) => {
  return res.json({ snapshot: computeFinanceSnapshot() });
});

// Async report request (SRS FR-32)
reportingRouter.post("/finance/reports", authRequired, requireRole(["finance", "admin"]), (req, res) => {
  ...
  // Local MVP: complete immediately. Later: queue job.
  try {
    const result = {
      ...
      from: report.from,
      to: report.to,
      snapshot: computeFinanceSnapshot()
    };
