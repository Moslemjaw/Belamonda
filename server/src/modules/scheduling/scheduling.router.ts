...
async function canActOnClinic(
  ...
  if (actor.role === "admin" || actor.role === "cs") return true;
...
schedulingRouter.get("/cs/requests", authRequired, requireRole(["cs", "admin", "clinicStaff", "finance"]), async (req, res) => {
...
schedulingRouter.post("/requests/:id/propose", authRequired, requireRole(["clinicStaff", "cs", "admin"]), async (req, res) => {
...
schedulingRouter.post("/requests/:id/confirm", authRequired, requireRole(["clinicStaff", "cs", "admin"]), async (req, res) => {
  const breq = await bookingRequestsStore.get(req.params.id);
  if (!breq) return res.status(404).json({ error: "NOT_FOUND" });
  if (!(await canActOnClinic({ userId: req.auth!.userId, role: req.auth!.role }, breq.clinicId))) {
    return res.status(403).json({ error: "FORBIDDEN_CLINIC" });
...
schedulingRouter.post("/requests/:id/reject", authRequired, requireRole(["clinicStaff", "cs", "admin"]), async (req, res) => {
...
schedulingRouter.post(
  ...
  authRequired,
  requireRole(["cs", "admin", "clinicStaff"]),
  async (req, res) => {
    const parsed = ProposeSchema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: "VALIDATION_ERROR" });
    const breq = await bookingRequestsStore.get(req.params.id);
...
schedulingRouter.get("/clinic/requests/:id/customer-context", authRequired, requireRole(["clinicStaff", "admin", "cs"]), async (req, res, next) => {
  try {
    const breq = await bookingRequestsStore.get(req.params.id);
    if (!breq) return res.status(404).json({ error: "NOT_FOUND" });
...
schedulingRouter.post("/clinic/sessions/:sessionId/reschedule", authRequired, requireRole(["clinicStaff", "admin", "cs"]), async (req, res, next) => {
  try {
...
schedulingRouter.post("/admin/user-offers/:uoId/adjust-sessions", authRequired, requireRole(["cs", "admin"]), async (req, res, next) => {
  try {
