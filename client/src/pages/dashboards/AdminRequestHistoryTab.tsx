...
export default function AdminRequestHistoryTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(e?.message || (ar() ? "فشل تحميل سجل الطلبات" : "Failed to load booking request history"));
      setItems([]);
    } finally {
      setLoading(false);
    }
      ...
      <div className="card-elevated bg-white border border-surface-200 p-5 space-y-4">
        ...
        {error && (
          <div className="p-4 rounded-xl bg-red-50 border border-red-200 text-red-700 flex items-center justify-between">
            ...
            <button onClick={() => load()} className="text-xs font-bold text-red-700 underline hover:text-red-900">
              ...
              {loading ? (
                Array.from({ length: 6 }).map((_, idx) => (
                  <tr key={`skeleton-${idx}`} className="animate-pulse">
                    <td><div className="h-4 bg-surface-200 rounded w-28 my-1" /></td>
                    <td>
                      <div className="h-4 bg-surface-200 rounded w-32 mb-1.5" />
                      <div className="h-3 bg-surface-100 rounded w-20" />
                    </td>
                    <td><div className="h-4 bg-surface-200 rounded w-28 my-1" /></td>
                    <td><div className="h-4 bg-surface-200 rounded w-28 my-1" /></td>
                    <td><div className="h-4 bg-surface-200 rounded w-36 my-1" /></td>
                    <td><div className="h-4 bg-surface-200 rounded w-20 my-1" /></td>
                    <td><div className="h-6 bg-surface-200 rounded-full w-20 my-1" /></td>
                  </tr>
                ))
