...
export function PromotionsManager() {
  ...
  const [form, setForm] = useState({ title: "", description: "", slug: "", offerIds: [] as string[] });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      refetchPromos();
      setForm({ title: "", description: "", slug: "", offerIds: [] });
    } catch (err: any) {
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-brand-pink-100">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button type="submit" disabled={saving || form.offerIds.length === 0} className="btn-primary">
            ...
            <div key={p._id || p.id} className={`card-elevated bg-white rounded-2xl p-5 border-2 ${p.isActive ? "border-brand-pink-200" : "border-surface-200 opacity-75"}`}>
              <div className="flex justify-between items-start gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-surface-600 mb-3">{p.description}</p>
