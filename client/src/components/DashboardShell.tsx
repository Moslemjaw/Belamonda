...
const Icons = {
  ...
  history: <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
...
const getGroupForNavItem = (key: string, role: string, isAr: boolean): { name: string; order: number } => {
  if (role === "admin") {
    ...
    if (["users", "clinics", "sessions_log", "request_history", "scan_history", "clinic_changes", "bookings", "reservations"].includes(key)) {
  ...
  if (role === "clinicStaff" || role === "clinic") {
    ...
    if (["invoices", "completed_sessions", "reports", "performance"].includes(key)) {
